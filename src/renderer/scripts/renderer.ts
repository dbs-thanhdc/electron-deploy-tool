...
function initAutoUpdateHandlers() {
  ...
  function showUpdateAvailablePopup(info: any) {
    ...
    // Update version info
    const newVersionText = document.getElementById('newVersionText') as HTMLElement;
    const latestVersionText = document.getElementById('latestVersionText') as HTMLElement;
    
    newVersionText.textContent = info.version || 'Unknown';
    ...
    if (info.releaseNotes) {
      ...
      releaseNotesContent.innerHTML = formatReleaseNotes(info.releaseNotes);
    }
  }
